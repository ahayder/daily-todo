...
function NowView({ day, now }: { day: PlannerDay; now: Date }) {
  ...
  const nowCard = (
    <div className="rounded-2xl border border-[var(--brand)] bg-[var(--paper-strong)] p-4">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--ink-700)]">
        Now{current ? ` · ${formatRange(current)}` : ""}
      </p>
      {current ? (
        <>
          <p className="mt-1 text-xl font-semibold text-[var(--ink-900)]">{current.title}</p>
          ...
          <p className="mt-2 text-sm text-[var(--ink-700)]">{formatLeft(minutesLeft(current, minutesNow))}</p>
        </>
      ) : (
        <>
          <p className="mt-1 text-xl font-semibold text-[var(--ink-900)]">Free — your call</p>
        ...
        <div className="flex flex-col gap-3">
          {segments.map((segment) => {
            if (segment.kind === "free") {
              ...
              return (
                <p key={`free-${segment.startMinutes}`} className="px-1 text-[13px] italic text-[var(--ink-700)] opacity-70">
            ...
            return (
              <p
                key={event.id}
                className={`px-1 text-[13px] ${isNext ? "text-[var(--ink-900)]" : "text-[var(--ink-700)]"}`}
              >
  ...
  return (
    <div className="mx-auto w-full max-w-md">
      <div className="mb-4 flex items-baseline justify-between">
        <p className="text-base font-semibold text-[var(--ink-900)]">Your ideal days</p>
        <span className="text-xs text-[var(--ink-700)]">edit once, rarely</span>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--line)] bg-[var(--paper-strong)] p-4">
        <div className="mb-4 flex items-center gap-4 border-b border-[var(--line)] pb-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-xs leading-relaxed text-[var(--ink-700)]">
        ...
        <button
          ...
          className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-[var(--line)] py-2 text-[13px] text-[var(--ink-700)] transition-colors hover:bg-[var(--paper)]"
        >
          <Plus className="h-[15px] w-[15px]" /> Add block
        </button>
      </div>

      <p className="mt-3 px-1 text-center text-xs text-[var(--ink-700)]">Changes save on their own.</p>
    </div>
  );
}
...
export function PlannerView({ state, dispatch }: Props) {
  ...
  if (!preset || !presetId) {
    return (
      <div className="flex h-full items-center justify-center p-6 text-sm text-[var(--ink-700)]">

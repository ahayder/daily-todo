...
- **Carryover**: When a new day is created (`ensureDailyPageForDate`), all incomplete todos and the markdown from the latest earlier daily page are copied forward automatically. Copied task IDs are deterministic, parent/subtask links are remapped to the new page, and PocketBase backfills the synthesized `daily_pages` record during hydration.
...
- **Daily planner persistence** — purposes live inside their owning planner day beside event slices in the existing `days_json` payload, so the radial planner does not require a PocketBase collection migration. A shared purpose id links manually copied weekday variants, while each copied slice receives its own event id.

...
- **Content Planner**: `/content-planner` is a full-width, single-board Kanban workspace. New boards seed Ideas, Planned, In Progress, Ready, and Published columns with editable titles and subtitles. Column titles act as both the rename control and the drag surface, without a separate drag icon; confirmation-gated column deletion lives in each column’s top-right three-dot menu, including an explanation when deletion is unavailable. Cards present one multiline Markdown surface and open a preview dialog when their surface or eye action is clicked; the preview’s existing content surface switches into direct Markdown editing from its top-right pencil action without opening or presenting a separate modal. The card’s pencil action remains the entry point for inline editing, which automatically expands a collapsed card. The first line is emphasized as the card title, expanded card bodies scroll at an approximately seven-line maximum height, and each card has its own ephemeral expand/collapse control. A compact bottom toolbar keeps preview, copy, edit, and expand/collapse actions separate without narrowing the title, while a far-left three-dot menu contains the confirmation-gated delete action. Copy always writes the complete title-first Markdown source, including notes hidden by card collapse, and briefly confirms success in place. The card surface provides pointer and keyboard dragging with a grab cursor; drag collision candidates are scoped by item type, and cards show an edge-aware insertion marker so the final position is predictable. Saved cards render safe GitHub-flavored Markdown, while the first line and remaining text continue syncing through the existing `title` and `notes` fields for compatibility.
...
- **Content planner is intentionally narrow** — one fixed board per user, editable columns, and locally collapsible Markdown cards with a preview dialog whose existing content surface switches into editing from a top-right pencil action, plus pencil-triggered inline card editing. Clicking the card surface previews it, while pointer dragging can start from that surface. The bottom toolbar exposes preview, full-Markdown copy, edit, and expand/collapse directly; deletion lives in its far-left three-dot menu and still requires confirmation. Copy uses the persisted title and notes even when the card body is collapsed. Editing automatically expands an inline-edited card. Card collapse and the active preview are ephemeral component state, not synced preferences. Card display uses `react-markdown` plus `remark-gfm` with raw HTML disabled, emphasizes the first persisted line as its title, and caps expanded card bodies at approximately seven text lines while card text still maps to `title` plus `notes` internally for persistence compatibility. Scheduling, taxonomy, scoring, filters, AI generation, and alternate views are outside this version.

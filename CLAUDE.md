...
│   ├── planner/            # 24-hour radial planner, purpose editor, and clock geometry
...
- **Daily planner persistence** — planner subtitle and creation metadata share the existing `days_json` payload with planner days, while purposes live inside their owning planner day beside event slices, so the radial planner does not require a PocketBase collection migration. Legacy day-only JSON is normalized on read. A shared purpose id links manually copied weekday variants, while each copied slice receives its own event id.

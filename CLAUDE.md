...
- **Content Planner**: `/content-planner` is a full-width, single-board Kanban workspace. New boards seed Ideas, Planned, In Progress, Ready, and Published columns with editable titles and subtitles. Cards present one multiline Markdown surface, open a read-only preview dialog when their surface or eye action is clicked, and enter inline editing only through the pencil action. Editing automatically expands a collapsed card. The first line is emphasized as the card title, expanded cards scroll at an approximately seven-line maximum height, and each card has its own ephemeral expand/collapse control. The card surface provides pointer dragging with a grab cursor, while the hover-revealed drag handle preserves keyboard-accessible ordering. Saved cards render safe GitHub-flavored Markdown, while the first line and remaining text continue syncing through the existing `title` and `notes` fields for compatibility.
...
- **Content planner is intentionally narrow** — one fixed board per user, editable columns, and locally collapsible Markdown cards with a read-only preview dialog plus pencil-only inline editing. Clicking the card surface previews it, while pointer dragging can start from that surface and the hover-revealed handle remains available for keyboard dragging. Editing automatically expands the card. Card collapse and the active preview are ephemeral component state, not synced preferences. Card display uses `react-markdown` plus `remark-gfm` with raw HTML disabled, emphasizes the first persisted line as its title, and caps expanded cards at approximately seven text lines while card text still maps to `title` plus `notes` internally for persistence compatibility. Scheduling, taxonomy, scoring, filters, AI generation, and alternate views are outside this version.
